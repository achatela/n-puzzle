...
int PuzzleSolver::linearConflict(std::vector<std::vector<int>> puzzle)
{
    ...
    for (int y = 0; y < _lineLength; y++) {
        std::map<int, std::list<int>> conflicts;
        for (int x = 0; x < _lineLength - 1; x++) {
            ...
            if (y != expectedPos.first)
                continue;
            for (int otherX = 0; otherX < _lineLength; otherX++) {
                int otherTile = puzzle[y][otherX];
                if (otherTile == 0)
                    continue;
                std::pair<int, int> otherExpectedPos = _snailPositions[otherTile];
                if (otherExpectedPos.first != y || (otherExpectedPos.second <= expectedPos.second && otherX <= x) || (otherExpectedPos.second >= expectedPos.second && otherX >= x))
                    continue;
                conflicts[tile].push_back(otherTile);
            }
        }
        for (auto conflict = conflicts.begin(); conflict != conflicts.end(); conflict = conflicts.erase(conflict), linearConflict += 2) {
            for (auto otherConflict = std::next(conflict); otherConflict != conflicts.end(); otherConflict++) {
                auto tile = std::find(otherConflict->second.begin(), otherConflict->second.end(), conflict->first);
                if (tile != otherConflict->second.end())
                    otherConflict->second.erase(tile);
            }
        }
    }
    ...
    for (int x = 0; x < _lineLength; x++) {
        ...
        for (int y = 0; y < _lineLength - 1; y++) {
            ...
            if (x != expectedPos.second)
                continue;
            for (int otherY = 0; otherY < _lineLength; otherY++) {
                int otherTile = puzzle[otherY][x];
                if (otherTile == 0)
                    continue;
                std::pair<int, int> otherExpectedPos = _snailPositions[otherTile];
                if (otherExpectedPos.second != x || (otherExpectedPos.first <= expectedPos.first && otherY <= y) || (otherExpectedPos.first >= expectedPos.first && otherY >= y))
                    continue;
                conflicts[tile].push_back(otherTile);
            }
        }
